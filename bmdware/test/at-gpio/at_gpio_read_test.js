...
function testGpioRead(testCompleteCallback) {
    if(!testShouldContinue) {
    ...
    async.series([
        function(callback) {
            utils.log(5, "Configure GPIO as input, pull up")
            expected_results.push(new Buffer('OK', 'ascii'))
            onDataCompleteCallback = callback
            bmdware_at.setGpioConfig(target_port, bmdware_at.AT_P0_06, bmdware_at.AT_DIRECTION_IN, bmdware_at.AT_PULL_UP, null)
        },
        function(callback) {
            utils.log(5, "Get configuration")
            expected_results.push(new Buffer(bmdware_at.AT_P0_06 + ' ' + bmdware_at.AT_DIRECTION_IN + ' ' + bmdware_at.AT_PULL_UP, 'ascii'))
            onDataCompleteCallback = callback
            bmdware_at.getGpioConfig(target_port, bmdware_at.AT_P0_06, null)
        },
        function(callback) {
            utils.log(5, "Read")
            expected_results.push(new Buffer(bmdware_at.AT_STATE_HIGH, 'ascii'))
            onDataCompleteCallback = callback 
            bmdware_at.readGpio(target_port, bmdware_at.AT_P0_06, null)
        },
        function(callback) {
            testResult = 'PASS'
            ...
            utils.log(5, "AT GPIO Read test done")
