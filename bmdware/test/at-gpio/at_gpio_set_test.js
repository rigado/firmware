...
function testGpioSet(testCompleteCallback) {
    ...
    //TODO: Update this test using stuff from the configure test
    async.series([
        function(callback) {
            utils.log(5, "Configure GPIO as output, no pull")
            expected_results.push(new Buffer('OK', 'ascii'))
            onDataCompleteCallback = callback
            bmdware_at.setGpioConfig(target_port, bmdware_at.AT_P0_04, bmdware_at.AT_DIRECTION_OUT, bmdware_at.AT_PULL_NONE, null)
        },
        function(callback) {
            utils.log(5, "Set GPIO HIGH")
            expected_results.push(new Buffer('OK', 'ascii'))
            onDataCompleteCallback = callback
            bmdware_at.writeGpio(target_port, bmdware_at.AT_P0_04, bmdware_at.AT_STATE_HIGH)
        },
        function(callback) {
            utils.log(5, "Set GPIO low")
            expected_results.push(new Buffer('OK', 'ascii'))
            onDataCompleteCallback = callback
            bmdware_at.writeGpio(target_port, bmdware_at.AT_P0_04, bmdware_at.AT_STATE_LOW)
        },
        function(callback) {
            testResult = 'PASS'
            ...
            utils.log(5, "AT GPIO Set test done")
