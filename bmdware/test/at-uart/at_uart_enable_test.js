...
function testUartEnable(testCompleteCallback) {
    if(!testShouldContinue) {
    ...
    async.series([
        function(callback) {
            utils.log(5, "Configure Uart Parity disable")
            expected_results.push(new Buffer(bmdware_at.RC_OK, 'ascii'))
            onDataCompleteCallback = callback
            bmdware_at.setUartParityEnable(target_port, false, null)
        },
        function(callback) {
            utils.log(5, "Configure pass-through UART disable")
            expected_results.push(new Buffer(bmdware_at.RC_OK, 'ascii'))
            onDataCompleteCallback = callback
            bmdware_at.setUartEnable(target_port, false, null)
        },
        // set P0.24 of the setup board to high to set P0.06 of the target board
        function(callback) {
