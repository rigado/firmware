...
/**@brief Function for handling DFU patch init packets.
 *
 * @return    NRF_SUCCESS on success, an error_code otherwise.
 */
uint32_t dfu_patch_init_pkt_handle(dfu_update_packet_t * p_packet);

/**@brief Function for handling DFU patch data packets. This function has a different signature
 *        because patch data is not forced to be a multiple of 4 bytes since it is not being
 *        directly written to flash.
 *
 * @param[in] p_data The data for the packet
 * @param[in] length The length of the data
 *
 * @return Patch status information
 */
int32_t dfu_patch_data_pkt_handle(uint8_t * p_data, uint32_t length);

/**@brief Function for handling DFU config packets.
 *
 * @return    NRF_SUCCESS on success, an error_code otherwise.
 */
...
/**@brief Function for retrieving memory shared across modules.
 *
 * @return Pointer to shared memory if available, NULL otherwise
 */
uint8_t * dfu_get_shared_mem(void);

/**@brief Function to release shared memory from use.
 *
 * Should ONLY be called from the module which requested the shared memory!!!
 */
void dfu_release_shared_mem(void);

/**@brief Accessor for flag that holds state of whether or not protection should be enabled
 */
