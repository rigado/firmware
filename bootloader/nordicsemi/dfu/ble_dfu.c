...
enum
{
    ...
    OP_CODE_CONFIG             = 9,                                             /**< Value of the Op code field for 'Configure' command.*/
...
#ifdef SDK12
    attr_char_value.init_len  = MAX_DFU_PKT_LEN - 3;
    attr_char_value.max_len   = MAX_DFU_PKT_LEN - 3;
#else
...
static uint32_t on_ctrl_pt_write(ble_dfu_t * p_dfu, ble_gatts_evt_write_t * p_ble_write_evt)
{
    ...
    else
    {
            ...
            uint32_t err_code = (sd_ble_gatts_rw_authorize_reply(p_dfu->conn_handle, &write_authorize_reply));
            if (err_code != NRF_SUCCESS)
            {
                return err_code;
            }
            ...
            uint32_t err_code = (sd_ble_gatts_rw_authorize_reply(p_dfu->conn_handle, &auth_reply));
            if(err_code != NRF_SUCCESS)
            {
                return err_code;
            }
    ...
    {
        case OP_CODE_CONFIG:
            ble_dfu_evt.ble_dfu_evt_type = BLE_DFU_CONFIG;

            p_dfu->evt_handler(p_dfu, &ble_dfu_evt);
            break;
		case OP_CODE_SYS_RESTART_DFU:
			ble_dfu_evt.ble_dfu_evt_type = BLE_DFU_RESTART;
        ...
        case OP_CODE_RECEIVE_PATCH_INIT:
            ble_dfu_evt.ble_dfu_evt_type = BLE_DFU_RECEIVE_PATCH_INIT_DATA;
        
            p_dfu->evt_handler(p_dfu, &ble_dfu_evt);
            break;
        
        case OP_CODE_RECEIVE_PATCH:
            ble_dfu_evt.ble_dfu_evt_type = BLE_DFU_RECEIVE_PATCH_DATA;
        
            p_dfu->evt_handler(p_dfu, &ble_dfu_evt);
            break;
        
        case OP_CODE_GET_PROTOCOL_VER:
        
            err = ble_dfu_response_send(p_dfu, BLE_DFU_PROTOCOL_VER_PROCEDURE, (ble_dfu_resp_val_t)API_PROTOCOL_VERSION);
            if(err != NRF_SUCCESS)
            {
                return err;
            }
...
#ifdef SDK12
		case OP_CODE_GET_MAX_MTU:
			err = ble_dfu_response_send(p_dfu, BLE_DFU_MAX_MTU_SIZE_PROCEDURE, (ble_dfu_resp_val_t)get_client_rx_mtu());
            if(err != NRF_SUCCESS)
            {
                return err;
            }
...
uint32_t ble_dfu_init(ble_dfu_t * p_dfu, ble_dfu_init_t * p_dfu_init)
{
    ...
    #else
        //41c80000-1756-4c30-93cc-a8fcc2fbd54b
        const ble_uuid128_t base_uuid128 =
        {
            {
                0x02, 0x02, 0xFB, 0xC2, 0xFC, 0xA8, 0xCC, 0x93,
                0x30, 0x4C, 0x56, 0x17, 0x00, 0x00, 0xc8, 0x41
            }
        }; 
    #endif
