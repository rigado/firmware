/* The SDK hci_mem_pool interface only supports a single instance.
   Since both the serial HCI driver and BLE transport want to use it,
   with different settings, we need another copy of the driver,
   with different names.

   This is stupid.  It would be better to rewrite the interface and
   its callers to handle multiple instances.
*/
...
typedef struct
{
    uint8_t  rx_buffer[BLE_RX_BUF_SIZE];                                /**< RX buffer memory array. */
...
static rx_buffer_elem_t  m_rx_buffer_elem_queue[BLE_RX_BUF_QUEUE_SIZE]; /**< RX buffer element instances. */
...
uint32_t BLE_hci_mem_pool_open(void)
{
    ...
    m_rx_buffer_queue.free_window_count    = BLE_RX_BUF_QUEUE_SIZE;
...
{
    static uint8_t tx_buffer[BLE_TX_BUF_SIZE];
        ...
        if (length <= BLE_RX_BUF_SIZE)
        {
            ...
            *pp_buffer                    =
            ...
            m_rx_buffer_queue.write_index =
                    (m_rx_buffer_queue.write_index + 1u) & (BLE_RX_BUF_QUEUE_SIZE - 1u);
...
uint32_t BLE_hci_mem_pool_rx_consume(uint8_t * p_buffer)
{
    ...
    if (m_rx_buffer_queue.free_available_count != 0)
    {
        ...
        do
        {
            ...
            {
                consume_index = (consume_index + 1u) & (BLE_RX_BUF_QUEUE_SIZE - 1u);
            }
        }
                ...
                (m_rx_buffer_queue.free_available_count != 0))
        {
            ...
            ++(m_rx_buffer_queue.free_window_count);
            start_index = (consume_index + 1u) & (BLE_RX_BUF_QUEUE_SIZE - 1u);
        }
    }
    else
    {
...
uint32_t BLE_hci_mem_pool_rx_data_size_set(uint32_t length)
{
    ...
    // "Making embedded systems: Elicia White".
    const uint32_t index = (m_rx_buffer_queue.write_index - 1u) & (BLE_RX_BUF_QUEUE_SIZE - 1u);
    ...
    if (m_rx_buffer_queue.read_available_count != 0)
    {
        ...
        *p_length                    =
        ...
        m_rx_buffer_queue.read_index =
            (m_rx_buffer_queue.read_index + 1u) & (BLE_RX_BUF_QUEUE_SIZE - 1u);
